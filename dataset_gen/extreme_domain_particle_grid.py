...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Convert multiple 3D particle datasets to Eulerian grids using ONE shared "
            "global extreme domain (xmin/xmax/ymin/ymax/zmin/zmax) computed across all "
            "provided datasets."
        )
    )
    parser.add_argument(
...
def gather_dataset_frames(data_dirs, max_frames):
    datasets = []
    for data_dir in data_dirs:
        ...
        datasets.append({"data_dir": data_dir, "files": files})
    return datasets
...
def main():
    ...
    print(f"Total frames used for global bounds: {len(all_files)}")

    bounds = compute_auto_domain_3d(all_files, args.margin_frac)
    xmin, xmax, ymin, ymax, zmin, zmax = bounds

    print("Global extreme domain bounds (3D):")
    ...
    for ds in datasets:
        ...
        for file_path in tqdm(files, desc=f"Grid {data_dir.name}", unit="frame"):
            data = np.load(file_path)
        ...
        dataset_meta_path = out_dir / "metadata_extreme_domain_3d_grid.json"
        with open(dataset_meta_path, "w", encoding="utf-8") as f:
            json.dump(dataset_meta, f, indent=2)

        global_metadata["datasets"].append(
            {
                "data_dir": str(data_dir),
                "out_dir": str(out_dir),
                "n_frames": int(len(files)),
                "metadata_file": str(dataset_meta_path),
            }
        )
        print(f"Dataset metadata: {dataset_meta_path}")

    global_meta_path = out_root / "metadata_extreme_domain_global.json"
    with open(global_meta_path, "w", encoding="utf-8") as f:
        json.dump(global_metadata, f, indent=2)

    total_written = sum(d["n_frames"] for d in global_metadata["datasets"])
    print(f"Done. Wrote {total_written} 3D grid frames across {len(datasets)} datasets.")
    print(f"Global metadata: {global_meta_path}")
